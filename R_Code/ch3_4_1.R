...
# 作図用のxの点を作成
x_1_vec <- seq(mu_truth_d[1] - 2 * sigma_dd[1, 1], mu_truth_d[1] + 2 * sigma_dd[1, 1], by = 0.25)
x_2_vec <- seq(mu_truth_d[2] - 2 * sigma_dd[2, 2], mu_truth_d[2] + 2 * sigma_dd[2, 2], by = 0.25)
x_point_mat <- cbind(
...
ggplot() + 
  ...
  labs(title = "Multivariate Gaussian Distribution", 
       subtitle = paste0("N=", N, ", mu=(", paste(mu_truth_d, collapse = ", "), ")", 
                         ", sigma=(", paste(round(lambda_dd, 5), collapse = ", "), ")"), 
      x = expression(x[1]), y = expression(x[2]))
...
# 作図用のxの点を作成
x_1_vec <- seq(mu_truth_d[1] - 4 * sigma_dd[1, 1], mu_truth_d[1] + 4 * sigma_dd[1, 1], by = 0.25)
x_2_vec <- seq(mu_truth_d[2] - 4 * sigma_dd[2, 2], mu_truth_d[2] + 4 * sigma_dd[2, 2], by = 0.25)
x_point_mat <- cbind(
  rep(x_1_vec, times = length(x_2_vec)), 
  rep(x_2_vec, each = length(x_1_vec))
)
