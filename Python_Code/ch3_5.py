...
for n in range(N):
    ...
    x_smp_n = np.append(x_smp_n, smp_x)
    
    # 出力値を計算
    tmp_y = np.dot(w_m.T, x_vector(x_smp_n[n:n+1], M_truth))
    tmp_y += np.random.normal(loc=0.0, scale=1 / lmd, size=1) # ノイズ成分
    y_1n = np.append(y_1n, tmp_y, axis=1)
    
    # xのベクトル作成
    x_mn = x_vector(x_smp_n[n:n+1], M)
...
for n in range(N):
    ...
    ims_x = ax.scatter(x_smp_n[0:n+1], y_1n[0, 0:n+1].flatten(), color='chocolate') # 観測データ
    
    # グラフの設定
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_ylim(min(y_line), max(y_line))
    ax.grid(True)
