...
# 作図用のxの点を作成
x_1_point = np.arange(mu_truth_d[0] - 4 * sigma_dd[0, 0], mu_truth_d[0] + 4 * sigma_dd[0, 0], 0.25)
x_2_point = np.arange(mu_truth_d[1] - 4 * sigma_dd[1, 1], mu_truth_d[1] + 4 * sigma_dd[1, 1], 0.25)
x_1_grid, x_2_grid = np.meshgrid(x_1_point, x_2_point)
x_1_line = x_1_grid.flatten()
x_2_line = x_2_grid.flatten()
...
true_model = np.array([
    multivariate_normal.pdf(
        x=[x_1, x_2], mean=mu_truth_d, cov=np.linalg.inv(lambda_dd)
    ) for x_1, x_2 in zip(x_1_line, x_2_line)
])
print(true_model)
...
# 作図用のmuの点を作成
mu_1_point = np.arange(mu_truth_d[0] - 100.0, mu_truth_d[0] + 100.0, 0.25)
mu_2_point = np.arange(mu_truth_d[1] - 100.0, mu_truth_d[1] + 100.0, 0.25)
mu_1_grid, mu_2_grid = np.meshgrid(mu_1_point, mu_2_point)
mu_1_line = mu_1_grid.flatten()
mu_2_line = mu_2_grid.flatten()
...
prior = np.array([
    multivariate_normal.pdf(
        x=[mu_1, mu_2], mean=m_d, cov=np.linalg.inv(lambda_mu_dd)
    ) for mu_1, mu_2 in zip(mu_1_line, mu_2_line)
])
print(prior)
...
posterior = np.array([
    multivariate_normal.pdf(
        x=[mu_1, mu_2], mean=m_hat_d, cov=np.linalg.inv(lambda_mu_hat_dd)
    ) for mu_1, mu_2 in zip(mu_1_line, mu_2_line)
])
print(posterior)
...
predict = np.array([
    multivariate_normal.pdf(
        x=[x_1, x_2], mean=mu_star_hat_d, cov=np.linalg.inv(lambda_star_hat_dd)
    ) for x_1, x_2 in zip(x_1_line, x_2_line)
])
print(predict)
...
# 作図用のmuの点を作成
mu_1_point = np.arange(mu_truth_d[0] - 50.0, mu_truth_d[0] + 50.0, 0.5)
mu_2_point = np.arange(mu_truth_d[1] - 50.0, mu_truth_d[1] + 50.0, 0.5)
mu_1_grid, mu_2_grid = np.meshgrid(mu_1_point, mu_2_point)
mu_1_line = mu_1_grid.flatten()
mu_2_line = mu_2_grid.flatten()
mu_dims = mu_1_grid.shape
...
# 作図用のxの点を作成
x_1_point = np.arange(mu_truth_d[0] - 2 * sigma_dd[0, 0], mu_truth_d[0] + 2 * sigma_dd[0, 0], 0.25)
x_2_point = np.arange(mu_truth_d[1] - 2 * sigma_dd[1, 1], mu_truth_d[1] + 2 * sigma_dd[1, 1], 0.25)
x_1_grid, x_2_grid = np.meshgrid(x_1_point, x_2_point)
x_1_line = x_1_grid.flatten()
x_2_line = x_2_grid.flatten()
x_dims = x_1_grid.shape
...
trace_predict = [[
    multivariate_normal.pdf(
        x=[x_1, x_2], mean=mu_star_d, cov=np.linalg.inv(lambda_star_dd)
    ) for x_1, x_2 in zip(x_1_line, x_2_line)
]]
...
print(np.array(trace_posterior[0]).shape)
print(mu_1_line.shape)
#%%
